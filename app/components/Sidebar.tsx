...
/**
 * BookBot left sidebar: logo, new conversation, search, and recent conversations list.
 */

const RECENT = [
  {
    id: "1",
    title: "Fantasy Book Picks",
    description: "Recommended some trending fantasy novels",
  ...
  },
  {
    ...
    description: "Found mystery novels with good ratings",
  ...
  },
  {
    ...
    description: "Classic literature recommendations",
    time: "2 days ago",
    active: false,
    icon: null,
  },
...
export default function Sidebar() {
  return (
    <aside className="bookbot-sidebar">
        ...
        <ul className="bookbot-conversation-list">
          {RECENT.map((c) => (
            <li key={c.id}>
              <button
                type="button"
                className={`bookbot-conversation-item ${c.active ? "active" : ""}`}
              >
                <span className={`bookbot-conv-icon ${c.icon ? "custom" : ""}`}>
                  {c.icon || "👤"}
                </span>
